	...
	function SoapServer(options, security){
		if(!options){
			options = {
				tns: 'server.soap.com'
			};
	...
	SoapServer.prototype.securityCheck = function (headers) {
		if (this.security === 'basicAuth') {
			let credentials = Buffer.from(headers.authorization.replace('Basic ', ''), 'base64');

			if (this.users[credentials]) {
				return true;
			}
		}
	...
	SoapServer.prototype.castArgument = function (inputName, requestArgument, options){
		...
		switch(options.type){
			case 'object':
				...
				return (requestArgument && requestArgument[0] ? requestArgument[0] : requestArgument);

			case 'number':
				if (isNaN(requestArgument)) {
					throw new SoapServiceException(SoapServiceException.INVALID_INPUT_TYPE, 'Invalid input [' + inputName + '] type ' + typeof requestArgument + ', expected ' + options.type);
				}
	...
	SoapServer.prototype.handleOperationRequest = function (service, soapRequest){
		...
		for(var operationRequestName in operationRequest){
			...
			for(var inputName in operation.inputs){
				...
				inputName = 'tns:' + inputName;
				if(!requestArguments[inputName]){
					if(input.required)
	...
	SoapServer.prototype.setUsers = function(users) {
		this.users = users;
	};

	SoapServer.prototype.handleRequest = function(req, res){
		if (req.url.indexOf('wsdl') === -1 && this.security && !this.securityCheck(req.headers)) {
	...
	SoapServer.prototype.handleSoapRequest = function(req, callback){
		...
		if (req.method == 'POST') {
			...
			if('undefined' == typeof service) {
				throw new SoapServerException(SoapServerException.SERVICE_NOT_FOUND, 'Service [' + req.path + '] not found');
			}
			...
			req.on('end', function () {
				...
				parseXml(body, function (err, soapRequest) {
					...
					try{
						var soapResponse = self.handleOperationRequest(service, soapRequest);
						//console.dir(soapResponse);
						responseXml += self.js2xml(soapResponse, null, 2);
					}
					catch(exception){
	...
	SoapServer.prototype.getWsdl = function(host, service){
		...
		var wsdl = '<?xml version="1.0" encoding="UTF-8"?><definitions\n';
